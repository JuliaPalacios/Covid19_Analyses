...
cat(paste('\nTotal number of all sequences from', country, ':', 
          length(country.ind), '\n'))
...
to.exclude <- which(1 - colSums(sum.mat[1:4, ]) > site.th)
...
## =======================================================
# Additional filtering and site subsetting goes here.


write.range <- reduce(IRanges(start=to.include.site, end=to.include.site))
...
country.out <- DNAStringSet(do.call(paste0, lapply(country.out, as.character)))
names(country.out) <- headers
cat('Writing file. \n')
writeXStringSet(country.out, file=save.f, append=FALSE)
