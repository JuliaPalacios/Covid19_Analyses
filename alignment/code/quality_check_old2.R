...
#Note: metadata not longer in nextstrain repository. It is expected to be downloaded from Gisaid
...
gisaid.all.f <- paste(data.dir, 'msa.fasta', sep='')
...
meta.pp1 <- read.delim('meta_gisaid_tmp.txt', header=FALSE, sep='\t', as.is=TRUE,
                       col.names=c('strain', 'gisaid_epi_isl', 'date', 'country'))
...
ns.meta.data <- read.delim(paste(data.dir, 'metadata.tsv', sep=''), 
...
if (!('EPI_ISL_402125' %in% meta.to.write$gisaid_epi_isl)) {
    system('cat genbank_MN908947.fasta gisaid_seq_pp_beast.fasta > aligned.fasta')
