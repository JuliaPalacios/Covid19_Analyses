...
plot(divergence,pch=19,col="blue")
```


Of the total `r ncol(data.matrix)` sites in the reference sequence, `r divsum[1]` are monomorphic sites, `r divsum[2]` are binary, and only `r divsum[3]+divsum[4]` have 3 or 4 types. Largest entropy is observed is in the first `r max(entropyl[entropyl[,1]>.3,1][entropyl[entropyl[,1]>.3,1]<1000])` bases and the last `r min(entropyl[entropyl[,1]>.3,1][entropyl[entropyl[,1]>.3,1]>20000])` bases.  

<!-- gridlength<-100 -->
<!-- bases<-seq(1,ncol(data.matrix),by=100) -->
<!-- ssites<-rep(0,length(bases)-1) -->
