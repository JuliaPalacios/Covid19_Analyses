...
```{r}
df.clusts.k <- data.frame(cluster = clusts.k,region=meta_fig$region,dt=dt,name=names(clusts.k))
...
df.clusts.k <- df.clusts.k %>% left_join(meta_fig %>% select(strain_beast,best_lat,best_lon) %>% distinct(),by=c("name"="strain_beast"))
```
...
for (i in 1:length(udt)) { 
  ...
  plt1 <- ggplot(data=world) + geom_sf() + 
    geom_point(data=df.clusts.k.summ,aes(x=best_lon,y=best_lat,col=factor(cluster),size=n.sample),alpha=0.6) + ylim(-50,60) + xlim(-125,150) + ggtitle(udt[i])
  print(plt1)
}
```
